...
from configs import (Books_Config, Challenge_Config, Chat_Config, ChessDB_Config, Engine_Config, Gaviota_Config,
...
class Config:
    ...
    def from_yaml(cls, yaml_path: str) -> 'Config':
        ...
        chat_config = cls._get_chat_config(yaml_config.get('chat', {}))
        ...
        return cls(yaml_config.get('url', 'https://mskchess.ru'),
                   yaml_config['token'],
                   engine_configs,
                   syzygy_config,
                   gaviota_config,
    ...
    def _check_sections(config: dict[str, Any]) -> None:
        ...
        sections = [
            ...
            ['chat', dict | None, 'Section `chat` must be a dictionary with indented keys followed by colons.']]
    ...
    @staticmethod
    def _get_chat_config(chat_section: dict[str, bool]) -> Chat_Config:
        if 'commands' not in chat_section:
            chat_section['commands'] = True

        if not isinstance(chat_section['commands'], bool):
            raise TypeError('`chat` subsection `commands` must be a bool.')

        return Chat_Config(chat_section['commands'])
