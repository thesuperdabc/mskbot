...
class Game_Manager:
    def __init__(self, api: API, config: Config, username: str) -> None:
        ...
        self.rematch_manager = Rematch_Manager(api, config, username)
    ...
    async def run(self) -> None:
        while self.is_running:
            ...
            # Check for pending rematch requests (only once per cycle)
            if (self.rematch_manager.pending_rematch and 
                not self.rematch_manager.rematch_offered):
                
                rematch_request = self.rematch_manager.get_rematch_challenge_request()
                if rematch_request:
                    self.request_challenge(rematch_request)
                    self.rematch_manager.rematch_offered = True  # Mark as offered
                    print(f"Rematch challenge queued for {rematch_request.opponent_username}")
                    
                    # Set a timeout to clear pending rematch if no response
                    asyncio.create_task(self._clear_pending_rematch_after_timeout(
                        rematch_request.opponent_username, 
                        self.rematch_manager.config.rematch.timeout_seconds + 10
                    ))
    ...
    async def _clear_pending_rematch_after_timeout(self, opponent_username: str, timeout_seconds: int) -> None:
        """Clear pending rematch after timeout if no response."""
        await asyncio.sleep(timeout_seconds)
        
        # Check if we're still waiting for this opponent
        if (self.rematch_manager.pending_rematch and 
            self.rematch_manager.pending_rematch == opponent_username.lower()):
            
            print(f'Rematch timeout with {opponent_username}, clearing pending rematch')
            self.rematch_manager.clear_pending_rematch()
            
            # Reset the rematch count for this opponent since they didn't respond
